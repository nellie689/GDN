...
class nop2d(nn.Module):
    def __init__(self, ModesFno, WidthFno, TSteps):
        super().__init__()
        self.nop = FNO2d(modes1=ModesFno, modes2=ModesFno, width=WidthFno)
        self.TSteps = TSteps
...
class FNO3d(nn.Module):
    def __init__(self, modes1=4, modes2=4, modes3=4, width=8):
        super().__init__()
...
class nop3d(nn.Module):
    def __init__(self, ModesFno, WidthFno, TSteps):
        super().__init__()
        self.nop = FNO3d(modes1=ModesFno, modes2=ModesFno, modes3=ModesFno, width=WidthFno)
        self.TSteps = TSteps
...
class nop3dsimple(nn.Module):
    ...
    def forward(self, low_dim_features): #x:[25, 2, 32, 32] 
        # print(low_dim_features.shape)  #[25, 2, 32, 32]   
        # assert 3>111
        v_seq_low_dim = [low_dim_features]
        fnov_low_dim = low_dim_features
        for t in range(1,self.TSteps):  #T:9  step:1
        ...
        # for t in range(0,self.TSteps):  #T:9  step:1
        #     fnov_low_dim= self.nop(fnov_low_dim)
        #     v_seq_low_dim.append(fnov_low_dim)
        # return v_seq_low_dim
